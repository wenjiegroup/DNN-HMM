function [ ] = B2_main_process_of_prediction( neighbour, sizes, M, numepochs)
%   Detailed explanation goes here
%   The previous script learns the models of DNN-HMM and related DNN, Kmeans-GMM-HMM and EM-GMM-HMM respectively, 
%   and outputs the time cost, training accurary and test accurary. DNN is a part of DNN-HMM.
%   This script uses the model learnt previously and predicts the state of
%   each bin (in our paper, the Repli-Seq bin size is 1 kb). The input
%   parameters should match the input parameters of the learning process.
...
%%load model learnt previously
size_flag=[];
for i=1:length(sizes)
    size_flag=strcat(size_flag,'_',num2str(sizes(i)));
end
load(strcat(dir_out,'/HMMSupervised_result_M',num2str(M),'_i',num2str(opts.numepochs),size_flag,'_n',num2str(neighbour),'.mat'));
